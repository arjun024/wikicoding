...
class URLPath(MPTTModel):
    ...
    #wc
    language =  models.CharField(max_length=512, verbose_name=_('article language'), 
    ...
    class Meta:
        ...
        unique_together = ('site', 'parent', 'language', 'slug')
    ...
    @classmethod
    def get_by_path(cls, path, language, select_related=False):
        """
        ...
        parent = cls.root()
        for slug in slugs:
            #URL_CASE_SENSITIVE must be case sensitive
            if settings.URL_CASE_SENSITIVE:
                child = parent.get_children().select_related_common().get(slug=slug, language=language)
                child.cached_ancestors = parent.cached_ancestors + [parent]
                parent = child

            else:
                #redundant
                print("BUG!! - URL SHOULD BE CASE SENSITITVE (urlpath.py)")
                child = parent.get_children().select_related_common().get(slug__iexact=slug, language__iexact=language)
                child.cached_ancestors = parent.cached_ancestors + [parent]
                parent = child
            level += 1
    ...
    def get_absolute_url(self):
        return reverse('wiki:get', kwargs={'language': self.language, 'path': self.path})
    ...
    @atomic
    @transaction_commit_on_success
    def create_article(cls, parent, slug, site=None, title="Root", language="_", article_kwargs={}, **kwargs):
        """Utility function:
        ...
        article.add_revision(ArticleRevision(title=title, **kwargs),
                             save=True)
        article.save()
        newpath = cls.objects.create(site=site, parent=parent, slug=slug, language=language, article=article)
