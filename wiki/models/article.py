...
from wiki.wc_pygments import codeformat
...
class Article(models.Model):
    ...
    def render(self, preview_content=None):
        ...
        #by default, related name of Foreign Key backwards relation for FOO is FOO_set
        article_urlpath = self.urlpath_set.all()
        article_language = article_urlpath[0].language
        if(article_language == '_'):
            return mark_safe(article_markdown(content, self,
                         preview=preview_content is not None))
        return mark_safe(codeformat(code=content, language=article_language))
