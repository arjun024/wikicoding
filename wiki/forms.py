...
class EditForm(forms.Form, SpamProtectionMixin):
    ...
    def clean(self):
        ...
        if cd['title'] != self.initial_revision.title:
            raise forms.ValidationError(ugettext('You cannot change article title. Please create a new article.'))
        if cd['content'] == self.initial_revision.content:
...
class SetTextInput(forms.TextInput):
    def __init__(self, *args, **kwargs):
        super(SetTextInput, self).__init__(*args, **kwargs)
    
    def render(self, *args, **kwargs):
        html = super(SetTextInput, self).render(*args, **kwargs)
        return mark_safe('<div class="input-group">%s</div>' % html)
...
class CreateForm(forms.Form, SpamProtectionMixin):
    ...
    title = forms.CharField(label=_('Title'),)
    # I will automate this - see make_slug() in the view
    ...
    def clean_slug(self):
        ...
        if settings.URL_CASE_SENSITIVE:
            ...
            #redundant - URL must be case sensitive
            print("BUG!! - URL SHOULD BE CASE SENSITITVE (forms.py)")
            slug = slug.lower()
            slug = slug.replace('-', '_')
    ...
    def clean(self):
        ...
        try:
            try_article = models.URLPath.get_by_path(path=slug, language=language, select_related=True)
        except models.URLPath.DoesNotExist: #we can safely go ahead with article creation
            return self.cleaned_data
        from django.contrib import messages
        messages.error(self.request, mark_safe("""<span style='color:rgb(197, 49, 49)'>Cannot create existing code article.<br/>
            Please edit the code article <a href='/wiki/%s/%s'>%s</a></span>"""
            % (language, slug, title)))
...
class UserCreationForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        '_revision', 'raw', 'plugin', 'user', 'log', 'pages', 'recent', 'robots.txt', 'policies', 'donate']
