...
class ArticleRawView(ArticleMixin, TemplateView):
    def get(self, request, *args, **kwargs):
        return HttpResponse(self.article.current_revision.content, content_type="text/plain")

    @method_decorator(get_article(can_read=True))
    def dispatch(self, request, article, *args, **kwargs):
        return super(ArticleRawView, self).dispatch(request, article, *args, **kwargs)
...
class Create(FormView, ArticleMixin):
    ...
    def get_form(self, form_class):
        ...
        initial['slug'] = slug
        initial['title'] = None if slug is None else slug.replace('_', ' ')
        initial['language'] = self.kwargs.get('language', None) #self.request.GET.get('slug', None)
        if slug is None:
            initial['language'] = 'as' # first one in the list of languages. @TODO don't hardcode. get lang_list[0]
        kwargs['initial'] = initial
        form = form_class(self.request, self.urlpath, **kwargs)
        language = self.kwargs.get('language', '_')
        from django import forms as djangoforms
        form.fields['language'].widget = djangoforms.Select(
            attrs={
                'name': "language",
                'title': _("Language")
            }
        )
        languages_sorted_list = [(l.language, l.language_displayname) for l in models.Language.objects.all().order_by('language_displayname')]
        languages_sorted_list.append(('_', '(text article)'));
        form.fields['language'].choices = languages_sorted_list
        return form
    ...
    def form_valid(self, form):
        ...
        try:
            slug = form.cleaned_data['title'].replace(" ", "_")
    ...
    def get_success_url(self):
        return redirect('wiki:get', language=self.language, path=self.newpath.path)
...
class Edit(FormView, ArticleMixin):
            ...
            if Form:
                if form_id == self.request.GET.get('f', None):
                    ...
                    if form.is_valid():
                        ...
                        if self.urlpath:
                            #lang= getattr(self, 'language', '_')
                            return redirect('wiki:edit', path=self.urlpath.path, language=self.language)
    ...
    def get_success_url(self):
        ...
        if self.urlpath:
            return redirect("wiki:get", language=self.urlpath.language, path=self.urlpath.path)
...
class ChangeRevisionView(RedirectView):
    ...
    def get_redirect_url(self, **kwargs):
        if self.urlpath:
            return reverse("wiki:history", kwargs={'language':self.urlpath.language, 'path':self.urlpath.path})
        else:
